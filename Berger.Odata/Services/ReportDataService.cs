...
namespace Berger.Odata.Services
{
    public class ReportDataService : IReportDataService
    {
        ...
        public async Task<IList<MTDBrandPerformanceReportResultModel>> MTDBrandPerformance(MTDBrandPerformanceSearchModel model)
        {
            ...
            var dataActual = _summaryPerformanceReportRepo.GetDataBySP<spMTDTBrnadPerformanceReport>("spGetMTDBrandPerformanceReports", new List<(string, object)>
            {
                ("Depots", string.Join(",",model.Depots)),
                ("Territories", string.Join(",",model.Territories)),
                ("Zones", string.Join(",",model.Zones)),
                ...
                ("Month", model.Month),
                ("VolumeOrValue", model.VolumeOrValue),
                ("BrandCategory", model.Type),
            }, 
            nameof(spMTDTBrnadPerformanceReport.Depots),
            nameof(spMTDTBrnadPerformanceReport.TempBrand));
            ...
            var brands = dataActual.Select(x => x.Brand)
            ...
            Func<spMTDTBrnadPerformanceReport, string, bool> predicateActual = (x, brand) => x.Brand == brand;
            ...
            if (model.Type == EnumBrandType.MTSBrands)
            {
                ...
                foreach (var item in dataActual)
                {
                    item.TempBrand = brandFamilyInfos.FirstOrDefault(x => x.MatarialGroupOrBrand == item.Brand)?
            ...
            foreach (var brand in brands)
            {
                ...
                var tillDateLyActual = dataActual.Where(x => predicateActual(x, brand))
                                                    .Sum(x => x.TillDateLYActual);

                var tillDateCyActual = dataActual.Where(x => predicateActual(x, brand))
                                                .Sum(x => x.TillDateCYActual);
                ...
                tempResult.Depots = dataActual.Where(x => predicateActual(x, brand))
                ...
                tempResult.LYMTD = dataActual.Where(x => predicateActual(x, brand))
                                                .Sum(x => x.LYMTD);
                tempResult.CMTarget = cyDataTarget.Where(x => predicateTarget(x, brand))
                                                    .Sum(x => model.VolumeOrValue == EnumVolumeOrValue.Volume
                                                    ? CustomConvertExtension.ObjectToDecimal(x.TargetVolume)
                                                    : CustomConvertExtension.ObjectToDecimal(x.TargetValue));
                tempResult.CMActual = dataActual.Where(x => predicateActual(x, brand))
                                                    .Sum(x => x.CMActual);
                tempResult.TillDateGrowth = _odataService.GetGrowth(tillDateLyActual, tillDateCyActual);
                tempResult.AskingPerDay = (tempResult.CMTarget - tempResult.CMActual) / currentDate.RemainingDays();
                tempResult.TillDatePerformacePerDay = tempResult.CMActual / currentDate.TillDays();

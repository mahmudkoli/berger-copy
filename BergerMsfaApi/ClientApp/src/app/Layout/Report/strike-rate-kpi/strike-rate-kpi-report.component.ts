...
@Component({
  ...
  templateUrl: './strike-rate-kpi-report.component.html',
  styleUrls: ['./strike-rate-kpi-report.component.css'],
})
export class StrikeRateKpiReportComponent implements OnInit, OnDestroy {
  // data list
  ...
  getDownloadDataApiUrl = (query) =>
    this.reportService.downloadStrikeRateOnBusinessCall(query);
  getData = (query) => this.reportService.getStrikeRateOnBusinessCall(query);
  ...
  searchOptionSettings: SearchOptionSettings = new SearchOptionSettings({
    searchOptionDef: [
      ...
      new SearchOptionDef({
        searchOption: EnumSearchOption.Month,
        isRequired: true,
      }),
      new SearchOptionDef({
      ...
      new SearchOptionDef({
        searchOption: EnumSearchOption.CustomerClassificationType,
        isRequired: true,
      }),
    ],
  });
  ...
  ptableColDefGenerate() {
    ...
    this.ptableSettings.tableColDef = Object.keys(obj).map((key) => {
      return {
        ...
        displayType: !key.toLowerCase().includes('billingPercentage')
    ...
    const self = this;
    this.ptableSettings.tableColDef.sort(function (a, b) {
      return (
        self.sortingArray.indexOf(a.internalName) -
        self.sortingArray.indexOf(b.internalName)
      );
    });
  }

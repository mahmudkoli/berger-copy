...
import { LogInReportQuery } from 'src/app/Shared/Entity/Report/ReportQuery';
...
export class LoginLogReportComponent implements OnInit, OnDestroy {
	...
	tableName: string = 'Log In Report';
	// renameKeys: any = {'userId':'// User Id //'};
	renameKeys: any = {
		'totalUseTime' : 'Total Use time (In Hour)'
	};
	...
	getDownloadDataApiUrl = (query) => this.reportService.downloadLogInReport(query);
	getData = (query) => this.reportService.getLogInReport(query);

	searchConfiguration() {
		this.query = new LogInReportQuery({
			page: 1,

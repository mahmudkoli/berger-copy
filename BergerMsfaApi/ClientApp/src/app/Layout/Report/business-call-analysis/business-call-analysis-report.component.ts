...
export class BusinessCallAnalysisReportComponent implements OnInit, OnDestroy {
  ...
  sortingArray: any[] = [
    ...
    'noOfCallActual',
    'achivement',
    'exclusiveNoOfCallTarget',
    'exclusiveNoOfCallActual',
    'exclusiveAchivement',
    'nonExclusiveNoOfCallTarget',
    'nonExclusiveNoOfCallActual',
    'nonExclusiveAchivement',
  ];
  ...
  constructor(
    ...
    private commonService: CommonService //private dynamicDropdownService: DynamicDropdownService
  ) {
  ...
  getDownloadDataApiUrl = (query) =>
    this.reportService.downloadBusinessCallAnalysis(query);
  ...
  ptableColDefGenerate() {
    ...
    this.ptableSettings.tableColDef = Object.keys(obj).map((key) => {
      return {
        ...
        displayType: key.toLowerCase().includes('achivement')

...
@Component({
    selector: 'app-project-collection-report',
    templateUrl: './project-collection-report.component.html',
    styleUrls: ['./project-collection-report.component.css']
})
export class ProjectCollectionReportComponent implements OnInit, OnDestroy {
	...
	tableName: string = 'Direct Project Collection Report';
	// renameKeys: any = {'userId':'// User Id //'};
	renameKeys: any = {
		'projectSapId' : 'Project SAP ID',
	...
	ptableColDefGenerate() {
		...
		this.ptableSettings.tableColDef = Object.keys(obj).map((key) => {
			return { headerName: this.commonService.insertSpaces(key), internalName: key, 
				type: typeof obj[key] === 'number' ? 'text' : null, displayType: typeof obj[key] === 'number' ? 'number-format-color' : null, 

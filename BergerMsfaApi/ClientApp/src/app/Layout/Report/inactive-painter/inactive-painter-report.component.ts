...
import { InactivePainterQuery, LeadGenerationDetailsQuery, PainterRegisterQuery } from 'src/app/Shared/Entity/Report/ReportQuery';
...
@Component({
    selector: 'app-inactive-painter-report',
    templateUrl: './inactive-painter-report.component.html',
    styleUrls: ['./inactive-painter-report.component.css']
})
export class InactivePainterReportComponent implements OnInit, OnDestroy {
	// data list
	query: InactivePainterQuery;
	...
	tableName: string = 'Inactive Painter Report';
	renameKeys: any = {
						// 'depotIdAndName': 'Depot Id & Name'
					};
	...
	getDownloadDataApiUrl = (query) => this.reportService.downloadInactivePainters(query);
	getData = (query) => this.reportService.getInactivePainters(query);
	
	searchConfiguration() {
		this.query = new InactivePainterQuery({
			page: 1,
